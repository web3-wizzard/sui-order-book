#[test_only]
module orderbookmodule::orders_lowval_tests_five {
    ...
    fun test_one_exact_bid_ask_match_with_low_value_(test: &mut Scenario) {
...

        {
            ...
            orders::add_bid_order<SUI, USDT>(447_000_000, &mut orderbook, mint<USDT>(447_000_000, ctx(test)), &clock, ctx(test));
...

        {
            let orderbook = test::take_shared<Orderbook<SUI, USDT>>(test);
            let clock = clock::create_for_testing(ctx(test));
            orders::add_bid_order<SUI, USDT>(446_000_000, &mut orderbook, mint<USDT>(446_000_000, ctx(test)), &clock, ctx(test));
            clock::destroy_for_testing(clock);
            test::return_shared(orderbook);
        };
...

        {
            let orderbook = test::take_shared<Orderbook<SUI, USDT>>(test);
            let clock = clock::create_for_testing(ctx(test));
            orders::add_ask_order<SUI, USDT>(447_000_000, &mut orderbook, mint<SUI>(2_000_000_000, ctx(test)), &clock, ctx(test));
            clock::destroy_for_testing(clock);
            test::return_shared(orderbook);
        };
